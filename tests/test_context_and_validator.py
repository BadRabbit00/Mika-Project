...
def writing_db(tmp_path):
    ...
    with database.connection() as connection:
        connection.execute("INSERT INTO sources(id, topic) VALUES ('s', 'security')")
        for node_id, name in (("a", "seccomp"), ("b", "container escape")):
            connection.execute("INSERT INTO nodes(id, name, summary) VALUES (?, ?, ?)",
                               (node_id, name, "GRAPH_SECRET"))
        connection.execute("INSERT INTO edges(src, dst, rel, source_id) "
                           "VALUES ('a', 'b', 'defends_against', 's')")
        for index in range(16):
            connection.execute("INSERT INTO narrative(at, kind, gist, topic) VALUES (?, ?, ?, ?)",
                               (AT - timedelta(hours=index + 1), "summary", f"technical-{index}", "security"))
            connection.execute("INSERT INTO narrative(at, kind, gist) VALUES (?, 'offtop', ?)",
                               (AT - timedelta(hours=index + 1), f"life-{index}"))
        connection.execute("INSERT INTO threads(opened_at, kind, text, topic, status, priority) "
                           "VALUES (?, 'confusion', 'TECH_THREAD', 'security', 'open', 2)", (AT,))
        connection.execute("INSERT INTO threads(opened_at, kind, text, status, priority) "
                           "VALUES (?, 'life', 'LIFE_THREAD', 'open', 1)", (AT,))
        connection.execute("INSERT INTO people_facts(person_id, fact, at) "
                           "VALUES ('someone', 'PRIVATE_FACT', ?)", (AT,))
    return database
...
def day():
    ...
    return world.day_context(AT, sleep=SleepWindow(AT.replace(hour=1), AT.replace(hour=9)),
                             sleep_debt=0, location=next(iter(world.locations)), road_roll=0.5)


@pytest.fixture
def builder(writing_db):
    return ContextBuilder(Path("prompts"), database=writing_db,
...
def test_context_isolation_offtop(builder, day):
    request = builder.build("write_offtop", kind="offtop", **writing_blocks(day, offtop_event=TEXT))
    ...
    assert all(secret not in combined for secret in ("GRAPH_SECRET", "technical-0", "TECH_THREAD", "PRIVATE_FACT", "seccomp"))
    with pytest.raises(ContextIsolationError):
        builder.build("write_offtop", kind="offtop", **writing_blocks(day, offtop_event="A new seccomp article"))


def test_unchanged_shared_persona_cannot_bypass_offtop_isolation(writing_db, day):
    builder = ContextBuilder(Path("prompts"), database=writing_db,
                             mood_model=MoodModel.from_config(Path("config"), epoch=AT))
    with pytest.raises(ContextIsolationError):
        builder.build("write_offtop", kind="offtop", **writing_blocks(day, offtop_event=TEXT))


def test_context_technical_memory_is_bounded_and_excludes_life(builder, day):
    request = builder.build("write_tech", kind="summary", **writing_blocks(day, topic="security"))
    combined = request.system + request.user
    ...
    assert all(secret not in combined for secret in ("life-0", "LIFE_THREAD", "PRIVATE_FACT"))
    assert all(phase not in combined for phase in ("menstrual", "ovulatory", "luteal"))


def test_curator_text_uses_user_role(builder, day):
    request = builder.build("write_tech", kind="correction", **writing_blocks(
        day, issue="CURATOR_ISSUE", correct="CURATOR_CORRECTION", wrong_post_gist="OLD_GIST"))
    assert "CURATOR_CORRECTION" in request.user and "CURATOR_CORRECTION" not in request.system
...
def test_people_facts_excluded_from_posts(builder, day):
    with pytest.raises(ValueError):
        builder.build("write_offtop", kind="offtop", **writing_blocks(day, offtop_event=TEXT, people_facts="PRIVATE_FACT"))


def test_unread_article_context_has_no_graph_contents(builder, day):
    request = builder.build("write_tech", kind="found", **writing_blocks(
        day, article_title="Article", article_source="Publisher", article_kind="paper",
        given_by="curator", topic="security"))
...
async def test_token_budget_enforced(builder, day):
    ...
    async with LocalLLM(transport=httpx.MockTransport(handle)) as llm:
        with pytest.raises(ContextOverflow):
            await builder.build_checked("write_offtop", llm=llm, kind="offtop", **writing_blocks(day, offtop_event=TEXT))
...
async def test_writer_validates_before_saving_and_keeps_attempts_stateless(builder, writing_db, day):
    valid = TEXT * 4
    llm = writer_llm(["<casual>" + "汉" * 220 + "</casual>", "<casual>" + valid + "</casual>"])
    writer = Writer(writing_db, llm, builder, OutputValidator(llm, echo_similarity=lambda a, b: 0))
    ...
    assert all("汉" not in call.args[0].system + call.args[0].user for call in llm.generate.call_args_list)
    with writing_db.connection() as connection:
        assert connection.execute("SELECT text FROM posts WHERE id=?", (result.id,)).fetchone()[0] == valid
        rows = connection.execute("SELECT status, error FROM runs ORDER BY rowid").fetchall()
        assert len(rows) == 2 and "cjk" in rows[0]["error"] and rows[-1]["status"] == "validated"
...
async def test_writer_kills_after_three_invalid_outputs(builder, writing_db, day):
    ...
    with writing_db.connection() as connection:
        row = connection.execute("SELECT state, text FROM posts WHERE id=?", (result.id,)).fetchone()
        assert row["state"] == "killed" and row["text"] is None


async def test_writer_respects_sleep_blackout(builder, writing_db):
    world = World.from_config(Path("config"))
    night = world.day_context(AT.replace(hour=3), sleep=SleepWindow(AT.replace(hour=1), AT.replace(hour=9)),
                              sleep_debt=0, location=next(iter(world.locations)), road_roll=0.5)
    llm = writer_llm([])
    writer = Writer(writing_db, llm, builder, OutputValidator(llm, echo_similarity=lambda a, b: 0))
