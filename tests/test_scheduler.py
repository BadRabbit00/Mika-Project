...
RHYTHM = {
    "sessions": {"weekday": [{"start": "10:00", "len_min": 90}, {"start": "19:30", "len_min": 120}],
...
def test_rhythm_uses_literal_lognormal_gaps_and_aware_windows():
    ...
    for _ in range(50):
        assert rhythm.gap_minutes(left) == min(40, max(4, right.lognormvariate(2.6, 0.7)))
...
async def test_blackout_is_checked_before_queue_and_again_before_execution():
    ...
    scheduler = ActivityScheduler(Rhythm.from_mapping(RHYTHM), jobs, blackout=blackout,
                                  clock=lambda: AT)
    blocked = True
...
async def test_session_event_limit_and_apscheduler_timezone():
    jobs = JobQueue()
    scheduler = ActivityScheduler(Rhythm.from_mapping(RHYTHM), jobs,
                                  blackout=lambda at: Blackout(False), clock=lambda: AT)
...
def test_thread_selection_follows_architecture_order_and_expires_old_threads():
    ...
    assert next_post_kind([Thread(5, "correction", AT - timedelta(days=6))], **args) == "found"
